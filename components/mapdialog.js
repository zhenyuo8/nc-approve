...
define(["utils","base"],function(utils,baseClass){
    var Component = function(config){
        ...
        var dialogBtnWrapper = $("<div style='margin: 0' class='yy-dialog-btn-wrapper displayflex "+btnDirection+"'></div>");
        this.dialogBackCover.bind("click",function(){
            _this.hide();
        });
        this.dialogWrapper.css({width:'100%;!important',top:'0%;!important'})
        ...
        if(createContentCallBack){
            ...
            this.contentTextDom = $("<div class=''>"+contentText+"</div>");
        ...
        if(config.buttons){
            ...
            for(var i=0,j=config.buttons.length;i<j;i++){
                ...
                var title =btn.title||utils.getLanguageText('button');
                var style = btn.style;
