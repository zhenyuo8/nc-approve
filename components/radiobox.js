define(["utils","base"],function(utils,baseClass){
    ...
    var Component = function(config){
        ...
        // 更改伪类样式 更改单选样式
        if(this.config.selectedRadioStyle && this.config.selectedRadioStyle.fontSize) {
            this.config.selectedRadioStyle.fontSize += '';
            if (this.config.selectedRadioStyle.fontSize.indexOf('px') === -1) {
                this.config.selectedRadioStyle.fontSize = this.config.selectedRadioStyle.fontSize + 'px';
            }
            $('head').find('style.yy-radio-head').remove();
            $('<style class="yy-radio-head">.yy-radio-selected .yy-radio:before {font-size: '+ this.config.selectedRadioStyle.fontSize +'!important;}</style>').appendTo('head');
        }
    };
