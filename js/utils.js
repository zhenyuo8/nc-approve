    ...
    function (md5) {
        ...
        function convertAdaptationValue(key, val, index, len, screenSize, style) {
            ...
            if (len === 1) {
                ...
                if (key == "fontSize" || key == "font-size"|| key == "line-height"|| key == "lineHeight") {
                    val = parseInt(val);
                    var bl = (screenSize.width / Re.baseSize.width);
                    // bl = bl<1?1:bl;
                    bl = bl > 1.5 ? 1.5 : bl;
                ...
                } else if ( key == "top" || key == "marginBottom" || key == "marginTop" || key == "bottom" || key == "maxHeight" || key == "max-height"   || key == "minHeight" || key == "min-height") {
        ...
        var Re = {
            ...
            timestampToTimeStr: function (timestamp, splitr) {
                timestamp = +new Date(timestamp);//如果参数timestamp不是时间戳的处理
                var Re = "";
                try {
                    timestamp = parseFloat(timestamp);
                    ...
                    if (time < 10 * 1) {
                        ...
                        Re = this.getLanguageText('justNow');
                    } else if (10 * 1 < time && time < 60 * 1) {
                        Re = time + this.getLanguageText('fewSec');
                    } else if ((time < 60 * 60) && (time >= 60 * 1)) {
                        s = Math.floor(time / 60);
                        Re = s + this.getLanguageText('fewMin');
                    } else if (nowDateInfo.year === dateInfo.year && nowDateInfo.day === dateInfo.day && nowDateInfo.month === dateInfo.month) {
                        Re = this.getLanguageText('today') + dateInfo.hourStr + ":" + dateInfo.minStr;
                    } else {
                        if (splitr) {
            ...
            getLanguageText:function (en) {
                var defaultLang = 'zh-CN',language;
                ...
                if(lang==='zh-CN'){
                    language={
                        ...
                        'NetworkError':'网络异常',
                        'reLoading':'重新加载',
                        'networkerrorLoading':'网络异常,尝试再次下拉刷新',
                        'type':'  类型  ',
                        'button':'按钮',
                        'pleaseSelect':'请选择',
                        'Monday':'一',
                    ...
                    language={
                        ...
                        'fewSec':' Secs ago',
                        'fewMin':' Mins ago',
                        'example':'For example: I agree with it,please execute!',
                        'cancel':'Cancel',
                        'confirm':'Confirm',
                        'reset':'Reset',
                        ...
                        'reLoading':'ReLoading',
                        'networkerrorLoading':'Network exception, try to pull down again refresh',
                        'type':'  Type  ',
                        'button':'Button',
                        'pleaseSelect':'Select',
                        'Monday':'Mon',
            ...
            dateCalculate:function (interval, number, date,signal) {
                switch (interval) {
                    case "d ": {
                        if(signal==='add'){
                            date=date+number*24*60*60*1000;
                        }else{
                            date=date-number*24*60*60*1000;
                        }
                        // return date;
            ...
            getQueryStringByName:function (tempurl,name) {
                var result=tempurl.match(new RegExp("[\?\&]"+name+"=([^\&]+)","i"));
